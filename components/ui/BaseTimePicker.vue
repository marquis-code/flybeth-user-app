<template>
  <div class="relative w-full font-body" ref="timePickerRef">
    ...
    >
      <div class="flex items-center gap-4">
        <div class="p-2.5 border border-brand-blue/10 rounded-xl bg-brand-blue/5 group-hover:bg-brand-blue group-hover:text-white transition-all duration-500">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" :class="isOpen ? 'text-white' : 'text-black transition-colors group-hover:text-white'" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <Transition name="time-pop">
      <div 
        v-if="isOpen"
        class="absolute z-[110] mt-3 w-full bg-white rounded-[2.5rem] shadow-[0_20px_60px_-15px_rgba(13,29,173,0.15)] border border-gray-200 p-8 transform origin-top"
      >
        <!-- Quick Options -->
        <h4 class="text-sm  text-brand-gray/50 uppercase  mb-4 px-2">Quick Select</h4>
        <div class="grid grid-cols-2 gap-3 mb-6">
          <button 
            v-for="quick in quickTimes" 
            :key="quick"
            @click="selectTime(quick)"
            class="py-3 px-4 rounded-2xl border border-gray-200 bg-white/50 text-xs  text-black hover:bg-brand-blue hover:text-white hover:shadow-lg transition-all duration-300"
          >
            {{ quick }}
          </button>
        </div>
        ...
        <h4 class="text-sm  text-brand-gray/50 uppercase  mb-4 px-2">Every Hour</h4>
        <div class="grid grid-cols-4 gap-2 max-h-[160px] overflow-y-auto custom-scrollbar px-1">
          <button 
            ...
            class="py-2.5 rounded-xl text-sm font-bold text-brand-gray hover:bg-brand-blue/5 hover:text-black transition-all"
            :class="{ 'bg-brand-blue/5 text-black ring-1 ring-brand-blue/10': modelValue === `${(hour-1).toString().padStart(2, '0')}:00` }"
          >
        ...
        <button 
          @click="isOpen = false"
          class="w-full mt-6 py-3 bg-brand-green text-white text-sm  rounded-full hover:bg-brand-green/90 transition-all shadow-lg uppercase "
        >
          Close
        </button>
      </div>
