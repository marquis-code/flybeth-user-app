<template>
  <button
    :class="[
      'inline-flex items-center justify-center  rounded-full transition-all duration-300 transform active:scale-95 focus:outline-none focus:ring-4',
      variantClasses,
      sizeClasses,
      block ? 'w-full' : '',
...
const variantClasses = computed(() => {
  switch (props.variant) {
    ...
    case 'secondary':
      return 'bg-brand-blue/5 text-gray-900 hover:bg-brand-blue/10 focus:ring-brand-blue/20 shadow-sm'
    case 'outline':
      return 'border-2 border-brand-blue/20 text-gray-900 bg-white hover:border-brand-blue hover:shadow-lg focus:ring-brand-blue/20'
    case 'ghost':
