  ...
  <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 hover:shadow-xl hover:border-brand-blue/20 transition-all duration-500 group">
      ...
      <div class="flex items-center space-x-4">
        <div class="h-12 w-12 rounded-xl bg-gray-50 p-2 flex items-center justify-center border border-gray-100 group-hover:border-brand-blue/10 transition-colors">
          ...
          <div v-else class="h-full w-full rounded-lg bg-brand-blue/10 flex items-center justify-center">
            <span class="text-brand-blue font-black text-xs">{{ (flight.airline || 'FL').slice(0, 2).toUpperCase() }}</span>
          </div>
        </div>
        <div>
    ...
    <div class="flex flex-col md:flex-row justify-between items-center gap-6">
      ...
      <div class="text-center md:text-left min-w-[80px]">
        <p class="text-xl font-black text-brand-blue">{{ formatTime(flight.departureTime) }}</p>
        <p class="text-xs font-black text-brand-gray/40 uppercase tracking-widest mt-1">{{ flight.origin }}</p>
      </div>
      ...
      <div class="flex flex-col items-center flex-grow px-4 md:px-12 w-full md:w-auto">
        <div class="flex items-center gap-2 mb-2">
          ...
          <p class="text-[11px] font-black text-brand-gray/60 uppercase tracking-widest">{{ formatDuration(flight.duration) }}</p>
        </div>
      ...
      <div class="text-center md:text-right min-w-[80px]">
        <p class="text-xl font-black text-brand-blue">{{ formatTime(flight.arrivalTime) }}</p>
        <p class="text-xs font-black text-brand-gray/40 uppercase tracking-widest mt-1">{{ flight.destination }}</p>
      </div>
    </div>
    ...
    <div class="mt-8 pt-6 border-t border-gray-50 flex items-center justify-between">
      ...
      <button 
        ...
        class="bg-brand-blue hover:bg-brand-blue/90 text-white px-8 py-3 rounded-xl font-black text-xs uppercase tracking-widest shadow-lg shadow-brand-blue/10 transition-all hover:scale-105 active:scale-95"
      >
        Select Flight
      </button>
    </div>
