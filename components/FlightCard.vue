  ...
  <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 hover:shadow-md transition-shadow duration-300">
    ...
    <div class="flex justify-between items-center mb-4">
      <div class="flex items-center space-x-3">
        <img v-if="flight.airline?.logo" :src="flight.airline.logo" :alt="flight.airline?.name" class="h-8 w-8 object-contain" />
        <div v-else class="h-8 w-8 rounded-full bg-brand-blue/10 flex items-center justify-center">
          <span class="text-brand-blue font-black text-xs">{{ (flight.airline?.name || flight.flightNumber || 'FL').slice(0, 2).toUpperCase() }}</span>
        </div>
        <span class="font-medium text-gray-900">{{ flight.airline?.name || flight.flightNumber || 'Flight' }}</span>
      </div>
      <div class="text-right">
        <p class="text-xl font-bold text-gray-900">${{ flight.price?.amount || flight.pricing?.baseRate || '—' }}</p>
    ...
    <div class="flex flex-col md:flex-row justify-between items-center text-sm">
      ...
      <div class="text-center md:text-left">
        ...
        <p class="text-gray-500">{{ flight.departure?.airport || flight.origin || '—' }}</p>
      </div>
      ...
      <div class="flex flex-col items-center flex-grow px-4 md:px-8 w-full md:w-auto my-4 md:my-0">
        ...
        <p class="text-green-600 mt-1" v-if="(flight.stops ?? 0) === 0">Nonstop</p>
      ...
      <div class="text-center md:text-right">
        ...
        <p class="text-gray-500">{{ flight.arrival?.airport || flight.destination || '—' }}</p>
      </div>
    </div>

    <!-- Action -->
    <div class="mt-6 flex justify-end">
       <UiBaseButton size="sm" @click="$emit('select', flight)">
          Select Flight
        </UiBaseButton>
    </div>
  </div>
