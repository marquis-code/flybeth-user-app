  ...
  >
      ...
      <div class="flex items-center space-x-4">
        <div class="h-12 w-12 rounded-xl bg-gray-50 p-2 flex items-center justify-center border border-gray-100 group-hover:bg-gray-100 transition-colors">
          ...
          <div v-else class="h-full w-full rounded-lg bg-gray-100 flex items-center justify-center">
            <span class="text-gray-900 font-bold text-sm">{{ (flight.airline || 'FL').slice(0, 2).toUpperCase() }}</span>
          </div>
        </div>
        <div>
    ...
    <div class="flex flex-col md:flex-row justify-between items-center gap-6">
      ...
      <div class="flex flex-col items-center flex-grow px-4 md:px-12 w-full md:w-auto">
        <div class="flex items-center gap-2 mb-2">
          ...
          <p class="text-sm font-bold text-brand-gray/60 tracking-[0.1em]">{{ formatDuration(flight.duration) }}</p>
        </div>
        <div class="w-full h-[1px] bg-gray-100 relative flex items-center justify-center">
           ...
           <div class="absolute left-1/2 -translate-x-1/2 bg-white px-3">
             <PlaneIcon class="h-4 w-4 text-gray-500 transform rotate-90" />
           </div>
        </div>
        <p class="text-sm font-bold mt-4 tracking-[0.2em] text-gray-800">
    ...
    <div class="mt-8 pt-6 border-t border-gray-50 flex items-center justify-between">
      <div class="flex items-center gap-6">
        <p v-if="flight.seatsAvailable" class="text-sm font-bold text-brand-orange/60 tracking-widest">
        ...
        <div v-if="flight.conditions?.refundable" class="flex items-center gap-1.5">
           ...
           <span class="text-sm font-bold text-brand-green tracking-widest">Refundable</span>
        </div>
      </div>
      <button 
        @click.stop="$emit('select', flight)"
        class="bg-gray-900 text-white px-6 py-2.5 rounded-xl font-bold text-sm tracking-widest transition-all hover:bg-black active:scale-95"
      >
        Select Flight
      </button>
    </div>
