<template>
  <div class="stay-details-step animate-in fade-in slide-in-from-bottom-4 duration-500">
    ...
    <div v-if="stay" class="stay-card bg-white rounded-3xl border border-gray-300">
      <div class="bg-brand-blue p-6 flex justify-between items-center">
         <div class="flex items-center gap-3">
            ...
            <div>
               <h2 class="text-lg  text-white leading-tight tracking-tight">{{ stay.hotelName || stay.name || stay.stay?.name }}</h2>
               <div class="text-[9px]  text-blue-100 tracking-widest opacity-80 ">Verified Premium Property</div>
            </div>
         </div>
      ...
      <div class="p-8 space-y-8">
        <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
          <div class="space-y-1">
            <span class="text-[9px]  text-gray-400 tracking-widest block ">Room Type</span>
          ...
          <div class="space-y-1 border-l border-gray-100 pl-6">
            <span class="text-[9px]  text-gray-400 tracking-widest block ">Board Type</span>
            <span class="text-sm  text-gray-900 leading-tight">{{ stay.rateType || 'Standard Rate' }}</span>
          </div>
          <div class="space-y-1 border-l border-gray-100 pl-6">
            <span class="text-[9px]  text-gray-400 tracking-widest block ">Check-in</span>
            <span class="text-sm  text-gray-900">{{ stay.checkInDate || 'Check-in info' }}</span>
          </div>
          <div class="space-y-1 border-l border-gray-100 pl-6">
            <span class="text-[9px]  text-gray-400 tracking-widest block ">Check-out</span>
        ...
        <div v-if="stay.cancellationPolicies?.length > 0" class="bg-gray-50 rounded-2xl p-6 border border-gray-100 border-dashed">
          <h3 class="text-sm  text-gray-900 tracking-widest mb-4 ">Cancellation Policy</h3>
          <div class="space-y-3">
            <div v-for="(policy, idx) in stay.cancellationPolicies" :key="idx" class="flex justify-between items-center text-xs font-bold">
              <span class="text-gray-500 tracking-tight">By {{ formatDate(policy.from) }}</span>
        ...
        <div class="flex items-start gap-4 p-4 bg-blue-50/50 rounded-xl border border-blue-100">
           ...
           <p class="text-sm font-bold text-gray-500 leading-relaxed ">Standard check-in is after 2:00 PM. Please have your identification ready for a seamless check-in experience.</p>
        </div>
      </div>
    </div>
    ...
    <div v-if="!isBundle" class="flex justify-end mt-4">
      <button @click="$emit('continue')" class="bg-brand-orange hover:bg-orange-600 text-white  px-12 py-4 rounded-xl transition-all tracking-widest text-sm active:scale-[0.98]">
