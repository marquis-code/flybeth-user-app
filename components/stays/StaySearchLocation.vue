<template>
  <div class="relative flex-1" ref="pickerRef">
    <div 
      @click="isOpen = !isOpen"
      class="flex-1 min-w-[280px] bg-white rounded-l-[1.75rem] px-8 py-4 group hover:bg-white transition-all cursor-pointer"
    >
      <p class="text-sm text-black font-bold  mb-2 opacity-50 group-hover:opacity-100 transition-opacity">Destination</p>
      <div class="flex items-center text-sm  text-black leading-none truncate">
        {{ modelValue || 'Where are you going?' }}
      </div>
    </div>

    <div v-if="isOpen" class="absolute top-full left-0 mt-4 z-[200] bg-white rounded-[2rem] shadow-[0_30px_90px_-20px_rgba(0,0,0,0.15)] border border-gray-200 w-[92vw] sm:w-[380px] p-8">
        <!-- Mobile Close Button -->
        <button @click="isOpen = false" class="md:hidden absolute top-4 right-4 h-9 w-9 flex items-center justify-center text-black bg-white rounded-full z-10 hover:text-black transition-colors">
        ...
        <div class="mb-6">
          <div class="relative group">
            <span class="absolute left-4 top-1/2 -translate-y-1/2 text-black group-focus-within:text-black transition-colors">
            ...
            <input 
              ...
              class="w-full pl-12 pr-4 py-4 bg-white border-none rounded-2xl focus:ring-2 focus:ring-gray-200 transition-all text-sm font-bold text-black placeholder:text-black"
              @click.stop
            />
          </div>
        </div>
        ...
        <div class="max-h-[380px] overflow-y-auto custom-scrollbar -mx-2 px-2">
          ...
          <div v-if="!searchQuery && recentSearches.length > 0" class="mb-6">
            <h3 class="px-4 py-2 text-sm  text-black  opacity-40">Previous searches</h3>
            <div 
          ...
          <div v-if="!searchQuery" class="mb-6">
             <h3 class="px-4 py-2 text-sm  text-black  opacity-40">Top destinations</h3>
             <div 
          ...
          <div v-if="searchQuery && results.length" class="space-y-1">
            ...
            >
              ...
              <div class="flex-grow min-w-0">
                <p class="text-sm  text-black group-hover:text-black transition-colors truncate">
                  {{ res.address?.cityName || res.name }}
                </p>
                <p class="text-sm font-bold text-black truncate  mt-0.5 opacity-60">
          ...
          <div v-if="isLoading" class="px-6 py-16 text-center">
            <div class="inline-block animate-spin h-10 w-10 border-4 border-gray-200 border-t-gray-900 rounded-full mb-6"></div>
            <p class="text-sm  text-black  opacity-40">Searching</p>
          </div>
        </div>
    </div>
  </div>
