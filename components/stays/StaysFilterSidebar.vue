<template>
  <div class="stays-sidebar bg-white rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.02)] border border-gray-200 p-8 sticky top-24">
    <div class="flex items-center justify-between mb-10 border-b border-gray-200 pb-8">
      <h2 class="text-xs   text-black">Filters</h2>
      <button @click="Object.assign(filters, {name: '', maxPrice: maxPrice, ratings: [], amenities: [], types: []})" class="text-[9px]  text-black  hover:text-black transition-colors">Reset all</button>
    </div>

    <!-- Search by Name -->
    <div class="mb-12">
      <div class="relative group">
        <span class="absolute left-5 top-1/2 -translate-y-1/2 text-black group-focus-within:text-black transition-colors">
        ...
        <input 
          ...
          class="w-full pl-14 pr-4 py-4 bg-white border-none rounded-2xl focus:ring-2 focus:ring-gray-200 transition-all text-xs font-bold text-black placeholder:text-black"
        />
      </div>
    </div>
    ...
    <!-- Budget Range -->
    <div class="mb-12">
      <h3 class="text-sm  text-black  mb-8">Price range</h3>
      <div class="px-2">
        <input 
          type="range" 
    ...
    <!-- Property Rating -->
    <div class="mb-12">
      <h3 class="text-sm  text-black  mb-8">Star rating</h3>
      <div class="grid grid-cols-1 gap-4">
        <label v-for="star in [5, 4, 3]" :key="star" class="flex items-center group cursor-pointer">
          <div class="relative flex items-center justify-center w-6 h-6 border-2 border-gray-200 rounded-xl mr-4 group-hover:border-gray-200 transition-all" :class="{'bg-black border-gray-200 shadow-xl shadow-gray-900/10': filters.ratings.includes(star)}">
          ...
          <div class="flex items-center">
            <span class="text-xs  text-black mr-3">{{ star }} Stars</span>
    ...
    <!-- Amenities -->
    <div class="mb-12">
      <h3 class="text-sm  text-black  mb-8">Amenities</h3>
      <div class="space-y-5">
        <label v-for="amenity in commonAmenities" :key="amenity.id" class="flex items-center justify-between group cursor-pointer">
          <div class="flex items-center">
            <div class="relative flex items-center justify-center w-6 h-6 border-2 border-gray-200 rounded-xl mr-4 group-hover:border-gray-200 transition-all" :class="{'bg-black border-gray-200 shadow-xl shadow-gray-900/10': filters.amenities.includes(amenity.id)}">
            ...
            <span class="text-[11px]  text-black  opacity-60 group-hover:opacity-100 transition-opacity">{{ amenity.label }}</span>
          </div>
          <span class="text-[9px]  text-black bg-white px-2.5 py-0.5 rounded-lg" v-if="counts[amenity.id]">{{ counts[amenity.id] }}</span>
        </label>
      </div>
    </div>
    ...
    <!-- Accommodation Types -->
    <div>
      <h3 class="text-sm  text-black  mb-8">Accommodation types</h3>
      <div class="space-y-5">
        <label v-for="type in accommodationTypes" :key="type" class="flex items-center group cursor-pointer">
          <div class="relative flex items-center justify-center w-6 h-6 border-2 border-gray-200 rounded-xl mr-4 group-hover:border-gray-200 transition-all" :class="{'bg-black border-gray-200 shadow-xl shadow-gray-900/10': filters.types.includes(type)}">
          ...
          <span class="text-[11px]  text-black  opacity-60 group-hover:opacity-100 transition-opacity">{{ type }}</span>
        </label>
      </div>
    </div>
  </div>
