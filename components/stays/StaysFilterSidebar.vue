<template>
  <div class="stays-sidebar bg-white rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.02)] border border-gray-100 p-8 sticky top-24">
    <div class="flex items-center justify-between mb-10 border-b border-gray-50 pb-8">
      <h2 class="text-xs  tracking-wide text-gray-900">Filters</h2>
      <button @click="Object.assign(filters, {name: '', maxPrice: maxPrice, ratings: [], amenities: [], types: []})" class="text-[9px]  text-gray-500 tracking-wide hover:text-black transition-colors">Reset all</button>
    </div>
    ...
    <!-- Budget Range -->
    <div class="mb-12">
      <h3 class="text-[10px]  text-gray-900 tracking-wide mb-8">Price range</h3>
      <div class="px-2">
        <input 
          type="range" 
        ...
        />
        <div class="flex justify-between mt-6 text-[10px]  text-gray-900 tracking-wide">
    ...
    <!-- Property Rating -->
    <div class="mb-12">
      <h3 class="text-[10px]  text-gray-900 tracking-wide mb-8">Star rating</h3>
      <div class="grid grid-cols-1 gap-4">
        <label v-for="star in [5, 4, 3]" :key="star" class="flex items-center group cursor-pointer">
          ...
          <div class="flex items-center">
            <span class="text-xs  text-gray-900 mr-3">{{ star }} Stars</span>
    ...
    <!-- Amenities -->
    <div class="mb-12">
      <h3 class="text-[10px]  text-gray-900 tracking-wide mb-8">Amenities</h3>
      <div class="space-y-5">
        <label v-for="amenity in commonAmenities" :key="amenity.id" class="flex items-center justify-between group cursor-pointer">
          <div class="flex items-center">
            ...
            <span class="text-[11px]  text-gray-900 tracking-wide opacity-60 group-hover:opacity-100 transition-opacity">{{ amenity.label }}</span>
          </div>
          <span class="text-[9px]  text-gray-500 bg-gray-50 px-2.5 py-0.5 rounded-lg" v-if="counts[amenity.id]">{{ counts[amenity.id] }}</span>
        </label>
      </div>
    </div>
    ...
    <!-- Accommodation Types -->
    <div>
      <h3 class="text-[10px]  text-gray-900 tracking-wide mb-8">Accommodation types</h3>
      <div class="space-y-5">
        <label v-for="type in accommodationTypes" :key="type" class="flex items-center group cursor-pointer">
          ...
          <span class="text-[11px]  text-gray-900 tracking-wide opacity-60 group-hover:opacity-100 transition-opacity">{{ type }}</span>
        </label>
      </div>
    </div>
  </div>
