<template>
  <div class="bg-white rounded-[2.5rem] p-5 md:p-6 border border-gray-200 shadow-sm hover:shadow-2xl hover:-translate-y-1 transition-all duration-500 group cursor-pointer flex flex-col md:flex-row gap-6 lg:gap-10">
    ...
    <div class="md:w-[320px] lg:w-[380px] h-60 md:h-auto shrink-0 relative flex items-center justify-center rounded-[2rem] bg-[#F8FAFC] border border-gray-200 overflow-hidden group-hover:bg-white transition-colors duration-500">
      ...
      <div class="absolute top-4 left-4 flex flex-col gap-2">
         <div v-if="cruise.rating" class="bg-white/95 backdrop-blur-md px-3 py-1.5 rounded-xl flex items-center gap-1.5 shadow-md border border-white/20">
            ...
            <span class="text-[11px] font-extrabold text-black">{{ cruise.rating }}/5</span>
         </div>
         <div class="bg-brand-blue text-white px-3 py-1.5 rounded-xl text-sm font-bold uppercase  shadow-lg shadow-brand-blue/20">
      ...
      <div class="absolute bottom-4 left-4">
         <div class="bg-white/95 backdrop-blur-md px-3 py-1.5 rounded-xl flex items-center gap-2 shadow-md border border-white/20">
            <span class="text-sm  text-black uppercase er">{{ cruise.cruiseLineName || 'Cruise Line' }}</span>
         </div>
      </div>
    </div>
    ...
    <div class="flex-1 flex flex-col justify-between py-1">
      <div>
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-5">
          <div>
            <h3 class="text-2xl font-bold text-black group-hover:text-brand-blue transition-colors leading-tight">{{ cruise.name }}</h3>
            <p class="text-xs font-bold text-black uppercase  mt-1.5 flex items-center gap-2">
          ...
          <div class="flex items-center gap-2">
             <div class="text-right hidden sm:block">
                <p class="text-[9px] font-bold text-black uppercase ">Ship</p>
                <p class="text-[11px] font-extrabold text-black">{{ cruise.shipName || 'Luxury Vessel' }}</p>
             </div>
          </div>
        </div>
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-3">
          <div class="flex items-center gap-2.5 p-3 rounded-2xl bg-white border border-gray-200">
            <CalendarIcon class="h-4 w-4 text-brand-blue" />
            <div class="flex flex-col">
               <span class="text-[9px] font-bold text-black uppercase">Departure</span>
               <span class="text-[11px] font-bold text-black truncate">{{ formatDate(cruise.departureDate) }}</span>
            </div>
          </div>
          <div class="flex items-center gap-2.5 p-3 rounded-2xl bg-white border border-gray-200">
            <UserIcon class="h-4 w-4 text-brand-blue" />
            <div class="flex flex-col">
               <span class="text-[9px] font-bold text-black uppercase">Guests</span>
               <span class="text-[11px] font-bold text-black">Up to 4</span>
            </div>
          </div>
          <div class="flex items-center gap-2.5 p-3 rounded-2xl bg-white border border-gray-200">
            <SparklesIcon class="h-4 w-4 text-brand-blue" />
            <div class="flex flex-col">
               <span class="text-[9px] font-bold text-black uppercase">Service</span>
               <span class="text-[11px] font-bold text-black">Premium</span>
            </div>
          </div>
          <div class="flex items-center gap-2.5 p-3 rounded-2xl bg-brand-green/5 border border-brand-green/10">
            ...
            <span class="text-sm font-bold text-brand-green uppercase ">All Inclusive</span>
          </div>
        </div>
      </div>
      ...
      <div class="mt-8 pt-6 border-t border-gray-200 flex items-center justify-between">
        ...
        <button 
          ...
          class="h-14 px-8 bg-brand-blue text-white rounded-2xl font-bold text-xs uppercase  hover:bg-brand-blue/90 shadow-xl shadow-brand-blue/20 active:scale-95 transition-all flex items-center gap-2"
        >
          View Deal
