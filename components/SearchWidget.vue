      ...
      <div v-if="currentTab === 'Stays'" class="space-y-8">
        ...
        <div v-if="stayMode === 'single'" class="grid grid-cols-1 md:grid-cols-4 gap-6">
          <div class="md:col-span-2">
            <LocationPicker label="Where to?" placeholder="Lake Buena Vista, FL" id="location" v-model="searchState.location" />
          </div>
          <div>
            <UiCustomDatePicker 
        ...
        <div v-else class="space-y-6">
          <div v-for="(hotel, index) in multiHotels" :key="index" class="p-8 bg-gray-50/30 rounded-[2rem] border border-gray-50 space-y-6 relative group">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <LocationPicker label="Where to?" placeholder="City or Property" :id="'hotel-loc-' + index" v-model="hotel.location" />
              <UiCustomDatePicker 
                v-model="hotel.dates" 
                mode="range" 
                label="Check-in - Check-out"
              ...
              <div class="flex flex-wrap items-center gap-6">
                <label v-for="opt in [{label: 'Add a flight', model: 'bundleFlight'}, {label: 'Add a car', model: 'bundleCar'}]" :key="opt.label" class="flex items-center text-xs font-bold text-brand-blue cursor-pointer group">
                  <div class="relative flex items-center mr-3">
                    <input type="checkbox" v-model="(bundles as any)[opt.model]" class="sr-only" />
                    <div 
                      class="w-5 h-5 rounded-lg border-2 transition-all duration-300 flex items-center justify-center"
                      :class="(bundles as any)[opt.model] ? 'bg-brand-green border-brand-green' : 'border-gray-200 group-hover:border-brand-green/50'"
                    >
                      <svg v-if="(bundles as any)[opt.model]" xmlns="http://www.w3.org/2000/svg" class="h-3 w-3 text-white" viewBox="0 0 20 20" fill="currentColor">
      ...
      <div v-if="currentTab === 'Flights'" class="space-y-8">
        ...
        <div v-if="flightMode !== 'multicity'" class="grid grid-cols-1 md:grid-cols-12 gap-6 items-end">
          <div class="md:col-span-4 relative">
             <LocationPicker label="Leaving from" placeholder="Lagos (LOS)" id="origin" v-model="flightSearchState.origin" />
          </div>

          <div class="md:col-span-4 relative">
             <LocationPicker label="Going to" placeholder="City or airport" id="destination" v-model="flightSearchState.destination" />
          </div>
        ...
        <div v-else class="space-y-6">
           <div v-for="(leg, index) in multiFlightLegs" :key="index" class="p-8 bg-gray-50/30 rounded-[2rem] border border-gray-50 relative group">
              ...
              <div class="grid grid-cols-1 md:grid-cols-12 gap-6 items-end">
                <div class="md:col-span-4 relative">
                   <LocationPicker :label="'Flight ' + (index + 1) + ' origin'" placeholder="Origin" :id="'leg-origin-' + index" v-model="leg.origin" />
                </div>
                <div class="md:col-span-4 relative">
                   <LocationPicker label="Destination" placeholder="Destination" :id="'leg-dest-' + index" v-model="leg.destination" />
                </div>
                <div class="md:col-span-4">
                  ...
                  <div class="space-y-8 px-2">
                     <div v-for="type in [
                       ...
                       <div class="flex items-center space-x-5">
                         ...
                         <span class="text-xl font-black w-8 text-center text-brand-blue">{{ (flightTravelers as any)[type.key] }}</span>
                         <button @click="updateFlightTravelers(type.key, 1)" class="h-12 w-12 rounded-2xl bg-gray-50 flex items-center justify-center font-black text-brand-gray hover:bg-brand-blue hover:text-white transition-all transform active:scale-90">+</button>
                       </div>
                     </div>
                  </div>
          ...
          <div class="flex flex-wrap items-center gap-10">
             <label v-for="addon in [{label: 'Add a place to stay', model: 'bundleFlightAddStay'}, {label: 'Add a car', model: 'bundleFlightAddCar'}]" :key="addon.label" class="flex items-center text-xs font-black uppercase tracking-widest text-brand-blue cursor-pointer group">
               <div class="relative flex items-center mr-4">
                 <input type="checkbox" v-model="(bundles as any)[addon.model]" class="sr-only" />
                 <div 
                   class="w-6 h-6 rounded-lg border-2 transition-all duration-300 flex items-center justify-center"
                   :class="(bundles as any)[addon.model] ? 'bg-brand-green border-brand-green shadow-lg shadow-brand-green/20' : 'border-gray-200 group-hover:border-brand-green/50'"
                 >
                   <svg v-if="(bundles as any)[addon.model]" xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-white" viewBox="0 0 20 20" fill="currentColor">
                     <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd" />
                   </svg>
                 </div>
               </div>
       ...
       <div v-if="currentTab === 'Cars'" class="space-y-8">
          ...
          <div class="grid grid-cols-1 md:grid-cols-12 gap-6 items-end">
             <div :class="[carMode === 'rental' ? 'md:col-span-4' : 'md:col-span-3']">
                <LocationPicker :label="carMode === 'rental' ? 'Pick-up' : 'Where from'" :placeholder="carMode === 'rental' ? 'City or airport' : 'Address or hotel'" id="car-origin" v-model="carSearchState.origin" />
             </div>

             <div v-if="carMode === 'rides' || showDifferentDropOff" class="md:col-span-3">
                <LocationPicker label="Where to" placeholder="Drop-off location" id="car-destination" v-model="carSearchState.destination" />
             </div>
                ...
                <div class="flex items-center px-6 py-3 bg-brand-green/5 border border-brand-green/10 rounded-2xl">
                   ...
                   <label v-for="addon in [{label: 'Hotel', model: 'bundleCarHotel'}, {label: 'Flight', model: 'bundleCarFlight'}]" :key="addon.label" class="flex items-center text-xs font-bold text-brand-blue cursor-pointer group mr-6 last:mr-0">
                     <div class="relative flex items-center mr-3">
                       ...
                       <div 
                         class="w-5 h-5 rounded-lg border-2 transition-all duration-300 flex items-center justify-center"
                         :class="(bundles as any)[addon.model] ? 'bg-brand-green border-brand-green' : 'border-gray-200 group-hover:border-brand-green/50'"
                       >
                         <svg v-if="(bundles as any)[addon.model]" xmlns="http://www.w3.org/2000/svg" class="h-3 w-3 text-white" viewBox="0 0 20 20" fill="currentColor">
                           <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd" />
                         </svg>
                       </div>
                     </div>
       ...
       <div v-if="currentTab === 'Packages'" class="space-y-8">
          ...
          <div class="grid grid-cols-1 md:grid-cols-12 gap-6 items-end">
            <div v-if="packageType !== 'hotel+car'" class="md:col-span-4 relative">
              <LocationPicker label="Where from?" placeholder="City or airport" id="package-origin" v-model="packageSearchState.origin" />
            </div>
            
            <div :class="[packageType === 'hotel+car' ? 'md:col-span-12 lg:col-span-8' : 'md:col-span-8 lg:col-span-4']">
              <LocationPicker label="Where to?" placeholder="Destination" id="package-destination" v-model="packageSearchState.destination" />
            </div>
...
// Autocomplete State
// Ensure results close when clicking outside
onMounted(() => {
