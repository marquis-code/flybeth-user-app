          ...
          <div class="hidden lg:flex items-center gap-2">
            ...
            <div class="relative group">
              <button class="flex items-center gap-2.5 px-3 py-1.5 rounded-lg text-black hover:bg-gray-50 transition-colors">
                ...
                <div class="flex flex-col items-start">
                  <span class="text-sm font-bold text-gray-500 uppercase leading-none">Call 24/7 Support</span>
                ...
                <div class="bg-white border border-gray-200 rounded-xl shadow-lg overflow-hidden">
                  ...
                  <div class="px-5 py-4 border-b border-gray-200">
                    <div class="flex items-center justify-between">
                      ...
                      <span class="inline-flex items-center gap-1.5 text-sm font-bold uppercase  text-emerald-600 bg-emerald-50 border border-emerald-100 px-2.5 py-1 rounded-full">
                  ...
                  <!-- <div class="px-4 pb-4">
                    <div class="flex items-center justify-between px-3.5 py-2.5 bg-white border border-gray-200 rounded-lg">
                      ...
                      <span class="text-[11px] font-bold  text-black uppercase bg-white border border-gray-200 px-2.5 py-1 rounded-md">DIAL10</span>
                    </div>
                  </div> -->
                </div>
              </div>
            ...
            <template v-if="user">
              <div class="flex items-center gap-4 pl-2">
                <div class="flex items-center gap-3 py-1 px-2 hover:bg-white rounded-lg transition-colors cursor-pointer group">
                  <div class="flex flex-col text-right">
                    <p class="text-[13px] font-bold text-black leading-none">{{ user?.firstName }} {{ user?.lastName }}</p>
                    <p class="text-sm font-bold text-black uppercase  mt-1">{{ user?.role?.name || 'Explorer' }}</p>
                  </div>
                ...
                <button
                  ...
                  class="flex items-center justify-center w-9 h-9 border border-gray-200 rounded-lg text-black hover:text-red-500 hover:border-red-100 hover:bg-red-50 transition-all duration-200"
                  title="Sign out"
                >
      ...
      <div :class="['fixed inset-0 z-[999] lg:hidden transition-all duration-300', isMobileMenuOpen ? 'visible' : 'invisible']">
        ...
        <div :class="['absolute inset-y-0 left-0 w-[280px] bg-white flex flex-col transition-transform duration-300', isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full']">
          ...
          <div class="flex-1 overflow-y-auto py-4">
            <p class="text-sm font-bold uppercase  text-black px-5 mb-2">Explore</p>
            <NuxtLink
              v-for="item in exploreLinks"
              :key="item.name"
              :to="item.path"
              @click="isMobileMenuOpen = false"
              class="flex items-center gap-3.5 px-5 py-3 hover:bg-white transition-colors"
            >
              <component :is="item.icon" class="h-4 w-4 text-black" />
              <span class="text-[14px] font-medium text-black">{{ item.name }}</span>
            </NuxtLink>

            <div class="mx-5 my-4 border-t border-gray-200"></div>

            <button @click="[showSettings = true, isMobileMenuOpen = false]" class="flex items-center gap-3.5 px-5 py-3 w-full hover:bg-white transition-colors text-left">
              <img :src="getFlag(locale)" class="h-3 w-4 object-cover rounded-sm" />
              <span class="text-[13px] font-medium text-black">{{ locale }} · {{ currentCurrency.code }}</span>
            </button>

            <a href="tel:+18443592384" class="flex items-center gap-3.5 px-5 py-3 hover:bg-white transition-colors">
              <Phone class="h-4 w-4 text-black" />
              <span class="text-[14px] font-medium text-black">Call us</span>
            </a>

            <NuxtLink to="/help" @click="isMobileMenuOpen = false" class="flex items-center gap-3.5 px-5 py-3 hover:bg-white transition-colors">
              <HelpCircle class="h-4 w-4 text-black" />
              <span class="text-[14px] font-medium text-black">Help center</span>
            </NuxtLink>
          </div>
          ...
          <div class="px-5 py-4 border-t border-gray-200">
            <div v-if="user" class="flex items-center justify-between">
              <div class="flex items-center gap-3">
                ...
                <div>
                  <p class="text-[13px] font-semibold text-black">{{ user?.firstName }} {{ user?.lastName }}</p>
                  <p class="text-sm font-bold uppercase  text-black">{{ user?.role?.name || 'Explorer' }}</p>
                </div>
              </div>
            ...
            <button v-else @click="[openAuthModal(), isMobileMenuOpen = false]" class="w-full bg-black text-white py-3 rounded-md font-semibold text-[13px] hover:bg-black transition-colors">
              Get Started
            </button>
          </div>
        </div>
