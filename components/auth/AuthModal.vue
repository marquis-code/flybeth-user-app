      ...
      <div v-if="isOpen" class="fixed inset-0 z-[1000000] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
        <div class="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-md overflow-hidden animate-scale-up border border-gray-100">
          ...
          <div class="p-8 pb-4 flex justify-between items-start">
            <div v-if="step !== 'otp'">
              <h2 class="text-2xl font-medium text-gray-900 tracking-tight font-spectral">{{ isLogin ? 'Welcome Back' : 'Create Account' }}</h2>
          ...
          <div v-if="step !== 'otp'" class="px-8 flex gap-8 border-b border-gray-50">
            <button @click="switchToLogin" class="pb-4 text-sm font-bold   transition-all relative" :class="isLogin ? 'text-brand-blue' : 'text-gray-400'">
              Login
              <div v-if="isLogin" class="absolute bottom-0 left-0 right-0 h-0.5 bg-brand-blue rounded-full"></div>
            </button>
            <button @click="switchToRegister" class="pb-4 text-sm font-bold   transition-all relative" :class="!isLogin ? 'text-brand-blue' : 'text-gray-400'">
              Register
              <div v-if="!isLogin" class="absolute bottom-0 left-0 right-0 h-0.5 bg-brand-blue rounded-full"></div>
            </button>
          </div>
          ...
          <div class="p-8 pt-6">
            ...
            <form v-if="step === 'login'" @submit.prevent="handleLogin" class="space-y-5">
              <div class="space-y-1.5">
                <label class="text-sm  text-gray-400   ml-1">Email Address</label>
              ...
              <button :disabled="authLoading" type="submit" class="w-full py-3 bg-gray-900 text-white rounded-2xl text-sm    hover:bg-black transition-all transform active:scale-[0.98] mt-4 shadow-xl shadow-gray-200 flex items-center justify-center gap-3">
            ...
            <div v-else-if="step === 'otp'" class="space-y-8 py-2">
              ...
              <div class="space-y-4 pt-2">
                ...
                <button 
                  ...
                  class="w-full text-sm  text-gray-400   hover:text-gray-900 transition-colors"
                >
            ...
            <div v-if="step !== 'otp'">
              <div class="relative py-8">
                ...
                <div class="relative flex justify-center text-sm  tracking-[0.4em]"><span class="bg-white px-6 text-gray-500 ">or</span></div>
              </div>

              <div class="space-y-4">
                <button :disabled="googleLoading" type="button" @click="loginWithGoogle" class="w-full flex items-center justify-center gap-4 px-5 py-4 border border-gray-300 rounded-2xl bg-white hover:bg-gray-50 transition-all text-sm  text-gray-900 shadow-sm group">
                ...
                <button type="button" @click="close" class="w-full flex items-center justify-center gap-2 px-5 py-4 border-2 border-dashed border-gray-200 rounded-2xl bg-white hover:bg-gray-50 hover:border-gray-300 transition-all text-sm font-bold text-gray-600 group">
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-gray-400 group-hover:text-gray-600 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>
