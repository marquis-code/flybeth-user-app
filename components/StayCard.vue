    ...
    <div class="p-4 md:w-2/3 flex flex-col justify-between">
      <div>
        <div class="flex justify-between items-start">
          <div>
            <h3 class="text-lg font-bold text-black">{{ stay.name || $t('stayCard.unnamedProperty') }}</h3>
            <p class="text-sm text-black location-text">{{ stay.location || stay.address || '' }}</p>
          </div>
        ...
        <div class="mt-2 text-sm text-black line-clamp-2">
          {{ stay.description || '' }}
        </div>
