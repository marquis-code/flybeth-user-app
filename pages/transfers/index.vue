    ...
    <div class="bg-white/80 backdrop-blur-md shadow-sm border-b border-gray-100 py-6 sticky top-0 z-30 transition-all duration-300">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row gap-6 items-end">
        <div class="flex-grow grid grid-cols-1 md:grid-cols-3 gap-4 w-full">
            <LocationPicker v-model="searchQuery.from" label="Pickup Location" placeholder="Airport, Hotel, etc." id="transfer-from" class="text-sm" />
            <LocationPicker v-model="searchQuery.to" label="Drop-off Location" placeholder="Destination" id="transfer-to" class="text-sm" />
            <div class="space-y-2">
              <UiAnimatedInput v-model="searchQuery.date" type="date" label="Pickup Date" placeholder="Select Date" class="!py-0" />
            </div>
        </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div class="lg:col-span-3 space-y-4">
          <div v-if="transfers.length" class="flex justify-between items-center mb-6">
            ...
            <div class="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-brand-gray/40">
...
const handleSearch = () => {
  searchTransfers(searchQuery.value)
}
...
const selectTransfer = (transfer: any) => {
  navigateTo({
    ...
    query: {
      type: 'transfer',
      id: transfer.id,
      name: `${transfer.vehicle_name} - ${transfer.provider_name}`,
      price: transfer.price?.amount || transfer.price || 0,
...
onMounted(() => {
  ...
  if (route.query.from) searchQuery.value.from = String(route.query.from)
  if (route.query.to) searchQuery.value.to = String(route.query.to)
