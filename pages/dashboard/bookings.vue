  ...
  <div class="min-h-screen bg-gray-50 pb-16">
      ...
      <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="flex items-center justify-between">
          ...
          <div class="flex items-center gap-3">
            <button
              @click="activeFilter = ''"
              :class="['px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all', !activeFilter ? 'bg-brand-blue text-white shadow-md' : 'bg-white text-gray-400 border border-gray-100 hover:border-brand-blue/30']"
            >All</button>
            <button
              @click="activeFilter = 'pending'"
              :class="['px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all', activeFilter === 'pending' ? 'bg-amber-500 text-white shadow-md' : 'bg-white text-gray-400 border border-gray-100 hover:border-amber-200']"
            >Pending</button>
            <button
              @click="activeFilter = 'confirmed'"
              :class="['px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all', activeFilter === 'confirmed' ? 'bg-green-500 text-white shadow-md' : 'bg-white text-gray-400 border border-gray-100 hover:border-green-200']"
            >Confirmed</button>
            <button
              @click="activeFilter = 'cancelled'"
              :class="['px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all', activeFilter === 'cancelled' ? 'bg-red-500 text-white shadow-md' : 'bg-white text-gray-400 border border-gray-100 hover:border-red-200']"
            >Cancelled</button>
          </div>
        </div>
      </div>
    ...
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div v-else-if="!filteredBookings.length" class="text-center py-24 bg-white rounded-2xl border border-gray-100 shadow-sm">
        ...
        <h2 class="text-xl font-black text-brand-blue mb-2">No Bookings Found</h2>
        ...
        <NuxtLink to="/flights" class="inline-flex items-center gap-2 px-8 py-3 bg-brand-blue text-white rounded-xl font-black text-sm uppercase tracking-widest hover:bg-brand-blue/90 transition-all shadow-lg shadow-brand-blue/20">
          ...
          <div class="p-6">
            ...
            <div v-if="booking.flights?.length" class="flex items-center gap-4 mb-5 p-4 bg-gray-50 rounded-xl">
              <div class="text-center">
                <p class="text-xl font-black text-brand-blue">{{ booking.flights[0]?.flight?.departure?.iataCode || '—' }}</p>
              ...
              <div class="flex flex-col items-center gap-1 flex-1">
                ...
                <span class="text-[9px] font-black text-gray-400 uppercase">{{ booking.flights[0]?.class || 'Economy' }}</span>
              </div>
              <div class="text-center">
                <p class="text-xl font-black text-brand-blue">{{ booking.flights[0]?.flight?.arrival?.iataCode || '—' }}</p>
            ...
            <div class="flex items-center justify-between">
              ...
              <div class="flex gap-2">
                <NuxtLink
                  ...
                  class="px-4 py-2 bg-gray-50 text-brand-blue rounded-xl text-xs font-black uppercase tracking-widest hover:bg-gray-100 border border-gray-100 transition-all"
                >
                  View Details
                </NuxtLink>
                <button
                  ...
                  class="px-4 py-2 bg-orange-500 text-white rounded-xl text-xs font-black uppercase tracking-widest hover:bg-orange-600 shadow-lg shadow-orange-200 transition-all"
                >
                  Pay Now
                </button>
              </div>
