<template>
  <div class="space-y-8">
    <div>
      <h2 class="text-sm  text-brand-green uppercase  mb-4">{{ $t('auth.authentication') }}</h2>
      <h3 class="text-4xl  text-black  er">{{ $t('auth.welcomeBack') }}</h3>
    ...
    <form class="space-y-6" @submit.prevent="handleLogin">
      ...
      <div class="flex items-center justify-between">
        <div class="text-sm">
          <NuxtLink to="/auth/forgot-password" class="font-bold text-black hover:text-black transition-colors">
      ...
      <div class="relative py-2">
        ...
        <div class="relative flex justify-center text-xs uppercase">
          <span class="bg-white px-4 text-brand-gray/40  ">{{ $t('auth.orContinueWith') }}</span>
        </div>
      </div>
