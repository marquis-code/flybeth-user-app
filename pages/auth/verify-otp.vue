<template>
  <div class="space-y-8">
    <div>
      ...
      <h3 class="text-4xl  text-black  er">Verify Email</h3>
      <p class="mt-3 text-sm text-brand-gray/60 font-bold">
        Enter the 6-digit code sent to 
        <span class="text-black ">{{ otpData.email }}</span>
      </p>
    </div>
    ...
    <form class="space-y-10" @submit.prevent="handleVerifyOtp">
      <div class="flex justify-between gap-2 px-1">
        <input 
          ...
          @paste="onOtpPaste"
          class="w-16 h-16 text-center text-2xl  bg-white border border-gray-200 rounded-2xl focus:ring-4 focus:ring-brand-blue/5 focus:border-brand-blue/30 outline-none text-black transition-all font-mono"
        />
      </div>
      ...
      <button
        ...
        class="w-full py-3 bg-brand-blue text-white rounded-2xl text-sm  uppercase  shadow-xl shadow-brand-blue/20 hover:shadow-brand-blue/40 transition-all active:scale-[0.98] disabled:opacity-50"
      >
        <span v-if="loading">Validating Secure Protocol...</span>
        <span v-else>Verify Identity</span>
      </button>
      
      <p class="text-center text-sm  text-black uppercase ">
